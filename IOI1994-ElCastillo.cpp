    ...
    while(!cola.empty())
    {
        ...
        for(int x =0; x<4; x++)
        {
            if(valor>=pos)
            {
                ...
                {
                    cola.push(aux);
                    visitados[aux.y][aux.x]=cuartos+1;
                    maximo++;
                }
            }
            pos/=2;
...
coordenadas MaximizarCuartos(int r,int c)
{
    ...
    while(!cola.empty())
    {
        ...
        for(int x =0; x<2; x++)
        {
            ...
            {
                if(arr[aux.y][aux.x]>0)
                {
                    cola.push(aux);
                }
                arr[aux.y][aux.x]=0;
                presente= visitados[uno.y][uno.x]-1;
                futuro=visitados[aux.y][aux.x]-1;
                if(presente!= futuro)
                {
