...
vll dijkstra(ll start, ll end,vector<vector<pair<ll,ll>>> graph,ll n)
{
...
void solve()
{
    ...
    vector<vector<pair<ll,ll>>> edges_inv(n+1);
    ...
    {   
        ...
        edges_inv[b].push_back({a,c});
        edges_list.push_back({a,b,c});
    }
    vll distances_n=dijkstra(1,n,edges,n);
    vll distances_r=dijkstra(n,1,edges_inv,n);
    ...
    {
        ll a,b,c;
        tie(a,b,c)=edges_list[x];
        ll join = distances_n[a]+ c/2 + distances_r[b];
        ans=min(ans,join);
    }
    cout<<ans<<endl;
}
