...
void solve()
{
    ...
    while(!cola.empty())
    {
        pair<ll,ll> a=cola.top();
        cola.pop();
        if(a.first>best[a.second].top())
            continue;
        for(pair<ll,ll> x:graph[a.second])
        {
            ll calc=x.second+a.first;
            if(best[x.first].size()<k)
            {
                best[x.first].push(calc);
                cola.push({calc,x.first});
            }
            else if(calc<best[x.first].top())
            {
                best[x.first].pop();
                best[x.first].push(calc);
                cola.push({calc,x.first});
