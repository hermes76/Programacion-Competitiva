...
ll maximum(vll &nums, ll n)
{
    ...
    for(ll x=1; x<=n; x++)
    {
        if(pila.top().first<=nums[x])
            pila.push({nums[x],x});
        else{
            ll index;
            while(!pila.empty() && pila.top().first>nums[x])
            {
                ll a,b;
                a=(x-pila.top().second) * pila.top().first;
                b=(x-pila.top().second+1)*nums[x];

                d=max(d,max(a,b));
                index=pila.top().second;
                pila.pop();
            }
            pila.push({nums[x],index});
        }
    }
    return d;
...
void solve()
{
    ...
    vector<vll> level(n+1,vll(m+1,0));
