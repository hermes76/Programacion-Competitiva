...
void solve()
{
    ...
    vll diferences;
    for(ll x=1; x<m; x++)
        diferences.push_back(nums[x]-nums[x-1]-1);
    diferences.push_back(n-nums[m-1] + nums[0]-1);
    sort(all(diferences));
    reverse(all(diferences));

    ll ans=0;
    ll day=0;
    
    for(ll x=0; x<diferences.size(); x++)
    {
        if(diferences[x]-day*2>=3)
        {
            ans+=min(day+ day+1, diferences[x]);
            day+=2;
        }
        else if( diferences[x]-day*2>0)
        {
                ans+=diferences[x]-1;
                day++;
         }
