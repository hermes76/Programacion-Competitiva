...
void solve()
{
    ...
    if(cad[0]=='a')
    {
        for(ll x=1; x<n-1; x++)
            if(cad[x]=='b')
            {
                cout<<cad.substr(0,x)<<' '<<cad.substr(x,n-x-1)<<' '<<cad[n-1]<<endl;
                return;
            }
        cout<< cad.substr(0,n-2)<<' '<< cad[n-2]<<' '<<cad[n-1]<<endl;
    }
    else{
        for(ll x=1; x<n-1; x++)
        {
            if(cad[x]=='a')
            {
                cout<<cad.substr(0,x)<<' '<<cad[x]<<' '<<cad.substr(x+1,n-x-1)<<endl;
                return;
            }
        }
        cout<< cad[0]<<' '<< cad.substr(1,n-2)<<' '<<cad[n-1]<<endl;
