...
void solve()
{
    ...
    for(ll x=0; x<n; x++)
    {
        order.insert({nums[x].first, nums[x].second, x});
    }
    vector< pair<ll,ll>> moves;
    for(ll x=0; x< n; x++)
    {
        auto it= order.begin();
        auto et= order.lower_bound({nums[x].first, nums[x].second,0});
        if(it==et)
        {
            order.erase(it);
            continue;
            ...
            auto aux= *et;
            get<2>(aux)= get<2>(*it);
            swap(nums[get<2>(*it)],nums[get<2>(*et)]);
            moves.push_back({get<2>(*it)+1,get<2>(*et)+1});
            order.erase(et);
            order.erase(it);
            order.insert(aux);
        }
    ...
    for(ll x=0; x<n; x++)
    {
        if(a>nums[x].first || b> nums[x].second)
            {cout<<-1<<endl; return;}
        a= nums[x].first;
        b= nums[x].second;
    }
