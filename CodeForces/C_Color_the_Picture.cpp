...
bool checking(ll rows,ll columns, ll k, vll & nums)
{
    ll count=0;
    if( columns== 3)
    {
        for(ll x=0; x<k; x++)
            if(nums[x]>= rows*columns)
                return true;
        return false;
    }
    bool great=false;
    ...
    if(columns%2==1 && count>= columns && great)
        return true;
    else if(columns%2==0 && count >=columns)
        return true;
    return false;
}
void solve()
{
    ...
    if(checking(n,m,k,nums) || checking(m,n,k,nums))
        cout<<"Yes";
    else cout<<"No";
    cout<<endl;
}
