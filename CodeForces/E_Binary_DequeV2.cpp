...
void solve()
{
    ...
    if(prefix[n-1]<s){cout<<-1<<endl;return;}
    if(prefix[n-1]==s){cout<<0<<endl; return;}
    ll res=INF;

    ll A=0, B=0;
    while(B<n && prefix[B]<=s)
        B++;
    res=n-1-(B-1);
    while(B<n)
    {
        while(A<n && prefix[B]-prefix[A]>s)
            A++;
        res=min(res, A+1+n-1-B);
        while(B<n && prefix[B]-prefix[A]<=s)
            B++;
        if(B==n)
            res=min(res,A+1);
        else
            res=min(res, A+1+n-1-(B-1));
    }
    cout<<res<<endl;
}
int main()
