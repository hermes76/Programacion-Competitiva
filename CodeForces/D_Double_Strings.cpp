...
void solve()
{
    ...
    for(ll x=0; x<n; x++)
    {
        string aux;
        bool band=false;
        for(ll y=0; cads[x][y]; y++)
        {
            aux+=cads[x][y];
            auto it=data.lower_bound(aux);
          //  cout<<cads[x]<<" "<<aux<<endl;
            if(it!=data.end() && *it== aux)
            {
                string aux2;
                for(ll z=y+1; cads[x][z]; z++)
                    aux2+=cads[x][z];
                //cout<<"PA "<<aux2<< " "<<aux<<endl;
                auto et= data.lower_bound(aux2);
               // cout<<*it<<" ";
               /* if(et!=data.end())

                    cout<<*et;
                cout<<endl;*/
                if(et!=data.end() && *et== aux2)
                {
                    band=true;
                    break;
                }
