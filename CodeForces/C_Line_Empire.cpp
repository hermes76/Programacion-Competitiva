...
void solve()
{
    ...
    ll best_solution=suffix[n] *b;
    ll index_a=0;
    ll left_part=0;
    for(ll x=1; x<=n; x++)
    {
       ll distance= nums[x]-nums[index_a];
       ll cost= distance*a;
       left_part += distance*b; 
       ll right_part = suffix[n]-(nums[x]*(n-x))- suffix[x];
       right_part*=b;
       if(cost+left_part+ right_part <= best_solution)
       {
            left_part+=cost;
            best_solution=left_part+right_part;
            index_a= x;
       }
    }
