...
void solve()
{
    ...
    for(ll x=1; x<=k; x++)
    {
        lengths[x]=last[x-1]+1;
        last[x]=moves[x-1].second-moves[x-1].first+1+last[x-1];
    }
    while(q--)
    {
        ll a; cin>>a;
        ...
        for(ll x=k; x>0; x--)
        {
          //  cout<< lengths[x]<<' '<<a<<' '<< moves[x-1].first-1<<endl;
            if(lengths[x]<=a)
                a-=lengths[x]-(moves[x-1].first-1);
        }
        cout<<cad[a]<<endl;
    }
}
