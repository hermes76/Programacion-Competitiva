...
void solve()
{
    ...
    multiset<ll> minimum;
    for(ll x=0; x<n-1; x++)
        minimum.insert(min(nums[x],nums[x+1]));
    ...
    for(ll x=0; x<n; x++)
    {
        ...
        order.erase({nums[x],x});

        nums[x]=INF;

        ll minix=*minimum.rbegin();
        if(x+1<n)
            minix=max(minix,min(nums[x],nums[x+1]));
        if(x-1>=0)
            minix=max(minix,min(nums[x],nums[x-1]));

        order.insert({nums[x],x});

        ll res=min(order.begin()->first*2,minix);
        //cout<<order.begin()->first*2<<' '<<minix<<endl;
        ans=max(ans,res);
