...
void solve()
{
   ...
   map<ll,ll> erased;
    ...
    {
        erased[nums[x]]++;
        checked.erase(nums[x]);
    }

    vll ans;
    set<ll> :: iterator it= checked.begin();
    ...
    for(ll x=0; x<n;)
    {
        ...
        it=checked.begin();
        ans.push_back(*it);
        vector<bool> diferent_numbers(*it,false);
        ll index=0;
        ll goal=*it;
        while(x<n && index<goal)
        {
            if(nums[x]<goal && diferent_numbers[nums[x]]==false)
            {
                diferent_numbers[nums[x]]=true;
                index++;
            }

             erased[nums[x]]--;
             if(erased[nums[x]]==0)
                checked.insert(nums[x]);
            x++;
        }
